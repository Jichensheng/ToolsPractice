...
//			virticleAnimator.setRepeatCount(ValueAnimator.INFINITE);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; authors != null && i < authors.length; i++) {
			...
			int out = canvas.save();
			//列级平移
			canvas.translate(i * (authorRectSize + paddingAuthor * 1.5f), 0);
			for (int j = 0; j <= authorSingle.length; j++) {
				int inside = canvas.save();
				//行级平移
				canvas.translate(0, j * (authorRectSize + paddingAuthor));
				if (j == 0) {
					mPaint.setColor(Color.parseColor("#9d9d9d"));
					drawCenterText(canvas, mPaint, " ", authorFontSizePx, paddingAuthor);
//					mPaint.setColor(Color.BLACK);
				} else
					drawCenterText(canvas, mPaint, authorSingle[j - 1], authorFontSizePx, paddingAuthor);
	...
	private void drawTitle(Canvas canvas) {
		...
		for (int i = 0; i < titles.length; i++) {
			canvas.save();
			canvas.translate(0, i * (titleRectSize + paddingTitle));
			drawCenterText(canvas, mPaint, titles[i], titleFontSizePx, paddingTitle);
